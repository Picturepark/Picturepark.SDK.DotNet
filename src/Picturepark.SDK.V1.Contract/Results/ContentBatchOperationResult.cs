...
namespace Picturepark.SDK.V1.Contract.Results
{
    public class ContentBatchOperationResult : BatchOperationResult<ContentDetail>
    {
        ...
        public async Task<BatchOperationResultDetail<ContentDetail>> FetchDetail(IEnumerable<ContentResolveBehaviour> resolveBehaviours = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            return await FetchDetail(
                async ids => await _contentClient.GetManyAsync(ids, resolveBehaviours, cancellationToken).ConfigureAwait(false),
                cancellationToken);
        }
    }
}