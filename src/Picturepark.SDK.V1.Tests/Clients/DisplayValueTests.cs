...
namespace Picturepark.SDK.V1.Tests.Clients
{
    ...
    public class DisplayValueTests : IClassFixture<ClientFixture>
    {
        ...
        {
            // Act
            var displayValue = await _client.DisplayValue.GetStatusAsync();
        ...
        public async Task ShouldRerenderDisplayValuesWhenStatusIsRed()
        {
            ...
            var schema = await SchemaHelper.CreateSchemasIfNotExistentAsync<DisplayPatternTest>(_client);
            ...
            var displayValueStatus = await _client.DisplayValue.GetStatusAsync();
            ...
            var businessProcess = await _client.DisplayValue.RerenderAsync();
            await _client.BusinessProcess.WaitForCompletionAsync(businessProcess.Id);

            // Assert
            content = await _client.Content.GetAsync(content.Id, new[] { ContentResolveBehavior.OuterDisplayValueThumbnail });
            content.DisplayValues[DisplayPatternType.Thumbnail.ToString().ToLowerCamelCase()].Should().Be($"{uniqueName}_{uniqueId}");

            displayValueStatus = await _client.DisplayValue.GetStatusAsync();
